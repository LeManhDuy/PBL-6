...
const ScheduleAdmin = () => {
    ...
    const [dropValueClass, setDropValueClass] = useState({value:0,label:0})
    ...
    const handleAddUpdatePeriod = (e) => {
        setErrorMessage("");
        console.log(e.target.textContent+"-"+e.target.id+"-"+e.target.name)
    ...
    const handleConfirmAddPeriod = (subject_teacher_id) =>{
        const schedule_id = allSchedule.find(x => x.class_id === dropValueClass.value).id
                    ...
                    {content_by_period.map((period,index) =>{
                        return(
                            <tr key={index+1}>
                                <td>{index+1}</td>
                                <td>
                                    <PeriodCell pnumber={index+1} date={'Mon'} handleClick={handleAddUpdatePeriod} period={period} />
                                </td>
                                <td>
                                    <PeriodCell pnumber={index+1} date={'Tue'} handleClick={handleAddUpdatePeriod} period={period} />
                                </td>
                                <td>
                                    <PeriodCell pnumber={index+1} date={'Wed'} handleClick={handleAddUpdatePeriod} period={period} />
                                </td>
                                <td>
                                    <PeriodCell pnumber={index+1} date={'Thu'} handleClick={handleAddUpdatePeriod} period={period} />
                                </td>
                                <td>
                                    <PeriodCell pnumber={index+1} date={'Fri'} handleClick={handleAddUpdatePeriod} period={period} />
                                </td>   
                            </tr>
                        )
                    })} 
    ...
    const handleDelete = () => {
        let deleting_schedule = allSchedule.find(x => x.class_id===dropValueClass.value)
        if(deleting_schedule){
        // console.log(schedule_tode)
            ScheduleService.DeleteScheduleById(deleting_schedule.id).then((res) => {
                if (res.success) {
                    setState(!state)
        ...
        <ModalCustom
            ...
            content={
                <ConfirmAlert
                    ...
                    title={`Do you want to delete this schedule for this class: `+ dropValueClass.label}
                />
            }
            handleCloseModalCustom={handleCloseModalCustom}
        />
    ...
    const handleConfirmAddSchedule = (props) => {
        ...
        ScheduleService.AddScheduleByFile(scheduleFile)
        .then((res) => {
            console.log(res)
            if (res.success) {
    ...
    const handleConfirmUpdatePeriod = (props) => {
        console.log(props)
        PeriodService.updatePeriod(props.id,props.params)
        .then((res) => {
            console.log(res)
            if (res.success) {
                setErrorServer(false);
    ...
    const handleConfirmDeletePeriod = (props) =>{
        console.log(props)
        PeriodService.deletePeriod(props)
        .then((res) => {
            console.log(res)
            if (res.success) {
                setErrorServer(false);
        ...
        <div className="main-container">
            <header>
                ...
                <div className="right-header">
                    <button className="btn-account" onClick={handleAddSchedule}>Add Schedule</button>
                    <button className="btn-dark" onClick={handleDeleteSchedule}>Delete Schedule</button>
            ...
            {deleteMessage? <label style={{color:'red'}}>Delete Successfully!</label> : null}
